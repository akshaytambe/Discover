...
if (isset($_POST['submitR'])) {
    if (!empty($replayInput)) {
        $status = "1";
        $replay_time = time();
        $sendReplay = $conn->prepare("UPDATE Feedback SET r_replay = :r_replay,r_replay_time = :r_replay_time,status = :status WHERE r_id = :rid");
...
<div class="dashboard_box" style="text-align: <? echo lang('textAlign'); ?>">
   ...
   <?php
        $n=0;
        $fetchReports = $conn->prepare("SELECT r_id,for_id,from_id,r_type,report,r_time FROM Feedback WHERE r_id =:rid");
        ...
        $fetchFrom = $conn->prepare("SELECT Fullname,Username,Userphoto FROM Users WHERE id = :from_id");
