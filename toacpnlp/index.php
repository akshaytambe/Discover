...
$cposts_q_sql = "SELECT post_id FROM Post";
...
$stars_q_sql = "SELECT id FROM Favorite_Friend";
$stars_q = $conn->prepare($stars_q_sql);
$stars_q->execute();
$stars_q_num_rows = $stars_q->rowCount();

$notes_sql = "SELECT id FROM DiaryNotes";
...
$saved_sql = "SELECT id FROM Saved_Post";
...
<div class="dashboard_box" style="text-align: <? echo lang('textAlign'); ?>">
    ...
    <table class="dashboard_sbFetch">
        ...
        $n=0;
        $status = "0";
        $fetchsubjects = $conn->prepare("SELECT r_id,from_id,r_type,subject,r_time FROM Feedback WHERE status = :status");
        ...
        $fetchFrom = $conn->prepare("SELECT Fullname,Username FROM Users WHERE id = :from_id");
