...
$uInfo = $conn->prepare("SELECT id,Fullname,Username,Email,Password,admin FROM Users WHERE Username = :ed");
...
if (isset($_POST['submit_uInfo'])) {
    ...
    $unExist = $conn->prepare("SELECT Username FROM Users WHERE Username =:db_username");
    ...
    $emExist = $conn->prepare("SELECT Email FROM Users WHERE Email =:db_email");
    ...
    if ($stop != "1") {
        ...
        $update = $conn->prepare("UPDATE Users SET Fullname = :db_fullname,Username = :db_username,Email = :db_email,admin = :db_admin WHERE Username = :ed");
        }else{
        $update = $conn->prepare("UPDATE Users SET Fullname = :db_fullname,Username = :db_username,Email = :db_email,Password = :db_password,admin = :db_admin WHERE Username = :ed");
        }
...
if (isset($_POST['rAccBtn'])) {
    if (empty($_POST['rAccField'])) {
        ...
        $remeveAccount_sql = "DELETE FROM Users WHERE id= :uInfo_id";
        ...
        $remeveAccount->bindParam(':uInfo_id',$uInfo_id,PDO::PARAM_STR);
        $remeveAccount->execute();
        $remeveAccount_sql = "DELETE FROM Following_Friends WHERE uf_one= :uInfo_id";
        $remeveAccount = $conn->prepare($remeveAccount_sql);
        $remeveAccount->bindParam(':uInfo_id',$uInfo_id,PDO::PARAM_STR);
        $remeveAccount->execute();
        $remeveAccount_sql = "DELETE FROM Following_Friends WHERE uf_two= :uInfo_id";
        $remeveAccount = $conn->prepare($remeveAccount_sql);
        $remeveAccount->bindParam(':uInfo_id',$uInfo_id,PDO::PARAM_STR);
        $remeveAccount->execute();
        ...
        $remeveAccount->bindParam(':uInfo_id',$uInfo_id,PDO::PARAM_STR);
        $remeveAccount->execute();
        $remeveAccount_sql = "DELETE FROM DiaryNotes WHERE author_id= :uInfo_id";
        $remeveAccount = $conn->prepare($remeveAccount_sql);
        $remeveAccount->bindParam(':uInfo_id',$uInfo_id,PDO::PARAM_STR);
        $remeveAccount->execute();
        $remeveAccount_sql = "DELETE FROM Favorite_Friend WHERE u_id= :uInfo_id";
        $remeveAccount = $conn->prepare($remeveAccount_sql);
        $remeveAccount->bindParam(':uInfo_id',$uInfo_id,PDO::PARAM_STR);
        $remeveAccount->execute();
        $remeveAccount_sql = "DELETE FROM Favorite_Friend WHERE p_id= :uInfo_id";
        $remeveAccount = $conn->prepare($remeveAccount_sql);
        $remeveAccount->bindParam(':uInfo_id',$uInfo_id,PDO::PARAM_STR);
        $remeveAccount->execute();
        $remeveAccount_sql = "DELETE FROM Post WHERE author_id= :uInfo_id";
        $remeveAccount = $conn->prepare($remeveAccount_sql);
        $remeveAccount->bindParam(':uInfo_id',$uInfo_id,PDO::PARAM_STR);
        $remeveAccount->execute();
        ...
        $remeveAccount->bindParam(':uInfo_id',$uInfo_id,PDO::PARAM_STR);
        $remeveAccount->execute();
        $remeveAccount_sql = "DELETE FROM Saved_Post WHERE user_saved_id= :uInfo_id";
        $remeveAccount = $conn->prepare($remeveAccount_sql);
        $remeveAccount->bindParam(':uInfo_id',$uInfo_id,PDO::PARAM_STR);
        $remeveAccount->execute();
        $remeveAccount_sql = "DELETE FROM Feedback WHERE from_id= :uInfo_id";
        $remeveAccount = $conn->prepare($remeveAccount_sql);
        $remeveAccount->bindParam(':uInfo_id',$uInfo_id,PDO::PARAM_STR);
        $remeveAccount->execute();
        if ($remeveAccount) {
...
<head>
    <title><? echo lang('dashboard'); ?> | Discover</title>
    <admin charset="UTF-8">
    <admin name="description" content="Discover is a social network platform helps you meet new friends and stay connected with your family and with who you are interested anytime anywhere.">
...
<div class="dashboard_box" style="text-align: <? echo lang('textAlign'); ?>">
    ...
    <?php
    $admin_int = "1";
    $chAdmin = $conn->prepare("SELECT Username FROM Users WHERE Username =:ed AND admin =:admin_int");
