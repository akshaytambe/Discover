...
<div class="main_container" align="center">
    <div style="display: inline-flex" align="center">
        ...
        $sid = $_SESSION['id'];
        $checkAuthOfPost_sql = "SELECT author_id FROM Post WHERE post_id = ?";
        ...
        while ($fetch_cop = $checkAuthOfPost->fetch(PDO::FETCH_ASSOC)) {
            ...
            if ($fetched_AuthorId == $sid) {
                $fPosts_sql = "SELECT * FROM Post WHERE post_id = ?";
                $params = array($post_id);
            }else{
                $checkFromPost_sql = "SELECT author_id FROM Post WHERE post_id = ? AND author_id IN (SELECT uf_two FROM Following_Friends WHERE uf_one= ?)";
                ...
                if ($checkFromPostCount < 1) {
                    $fPosts_sql = "SELECT * FROM Post WHERE post_id = ? AND p_privacy != ? AND p_privacy != ?";
                    $params = array($post_id, "1", "2");
                }else{
                    $fPosts_sql = "SELECT * FROM Post WHERE post_id = ? AND p_privacy != ?";
