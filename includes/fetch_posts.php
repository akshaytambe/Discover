...
$get_post_location = $postsfetch['p_location'];
...
$chShare = $conn->prepare("SELECT shared FROM Post WHERE shared=:get_post_id");
...
$body = nl2br("$body");
    ...
    <input type='text' dir='auto' class='flat_solid_textfield' id='EditLocationBox_$get_post_id' style='min-height: auto;' placeholder='".lang('w_location_inputText')."' value='$p_location' />
...
$fetch_shared = $conn->prepare("SELECT * FROM Post WHERE post_id=:get_post_shared ");
...
while ($sharedRow = $fetch_shared->fetch(PDO::FETCH_ASSOC)) {
    ...
    $shP_location = $sharedRow['p_location'];
