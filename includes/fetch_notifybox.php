...
$notify_from = $conn->prepare("SELECT Fullname,Username,Userphoto FROM Users WHERE id=:notify_from_id");
...
switch ($notifyType) {
case 'like':
$postBody = $conn->prepare("SELECT post_content FROM Post WHERE post_id=:notifyType_id");
...
break;
case 'star':
$getUsername = $conn->prepare("SELECT Username FROM Users WHERE id=:notifyType_id");
