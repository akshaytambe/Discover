...
$users = $conn->prepare("SELECT uf_two FROM Following_Friends WHERE uf_one=:myid");
...
$uf_two = $uf['uf_two'];
$usersInfo = $conn->prepare("SELECT id,online,Fullname,Userphoto,Username,verify FROM Users WHERE id=:uf_two");
...
$m_from = $uf2['m_from'];
$usersInfo = $conn->prepare("SELECT id,online,Fullname,Userphoto,Username,verify FROM Users WHERE id=:m_from");
...
$uReqCheck = $conn->prepare("SELECT uf_two FROM Following_Friends WHERE uf_one=:myid AND uf_two = :m_from");
...
$uSearch = $conn->prepare("SELECT id,online,Fullname,Userphoto,Username,verify FROM Users WHERE id IN (SELECT uf_two FROM Following_Friends WHERE uf_one= ?) AND (Fullname LIKE ? OR Username LIKE ?)");
...
$getInfo = $conn->prepare("SELECT Username,Fullname,Userphoto,online,bio,verify FROM Users WHERE id=:uid");
...
while ($msgs_row = $getMsgs->fetch(PDO::FETCH_ASSOC)) {
	$toUserQuery = $conn->prepare("SELECT Userphoto FROM Users WHERE id = :uid");
...
case 'checkTyping':
$uid = filter_var(htmlentities($_POST['uid']),FILTER_SANITIZE_NUMBER_INT);
$typing = $conn->prepare("SELECT t_from FROM Typing_Message WHERE t_from=:uid AND t_to = :myid");
...
case 'mTyping':
$uid = filter_var(htmlentities($_POST['uid']),FILTER_SANITIZE_NUMBER_INT);
$typingExist = $conn->prepare("SELECT t_from FROM Typing_Message WHERE t_from=:myid AND t_to = :uid");
...
$setTyping = $conn->prepare("INSERT INTO Typing_Message (t_from,t_to) VALUES (:myid,:uid)");
...
case 'mUnTyping':
$uid = filter_var(htmlentities($_POST['uid']),FILTER_SANITIZE_NUMBER_INT);
$unTyping = $conn->prepare("DELETE FROM Typing_Message WHERE t_from=:myid AND t_to = :uid");
