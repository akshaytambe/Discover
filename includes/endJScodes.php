...
var pl = $.trim($('#EditLocationBox_'+pid).val());
var pp = $.trim($('#p_privacy_'+pid).val());
var path = "<?php echo $checkDir; ?>";
$.ajax({
type: "POST",
url: "<?php echo $checkDir; ?>includes/updatepost.php",
data: {"pid":pid, "pc":pc, "pt":pt, "pl":pl, "pp":pp, "cp":checkPath},
cache: false,
dataType: "json",
beforeSend: function(loading){
...
$('#postLocation_'+pid).html("<p class='postLocation' style='border-<?php echo lang('float'); ?>: 2px solid #bee1ff; text-align:<?php echo lang('textAlign'); ?>;'>"+res[2]+"</p>");
$('#p_privacyView_'+pid).html(res[3]);
$('#postContent_'+pid).show();
if (res[1].length > 0) {$('#postTitle_'+pid).show();}
if (res[2].length > 0) {$('#postLocation_'+pid).show();}
