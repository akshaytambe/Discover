...
$s_id = $_SESSION['id'];
$user_id = $row_id;
$checkIfollowedHim_sql = "SELECT * FROM Following_Friends WHERE uf_one=:s_id AND uf_two=:user_id";
...
$vpssql = "SELECT * FROM Post WHERE author_id=:row_id AND p_privacy != :pr_onlyme  ORDER BY post_time DESC LIMIT :plimit,10";
...
$vpssql = "SELECT * FROM Post WHERE author_id=:row_id AND p_privacy != :pr_onlyme AND p_privacy != :pr_followers ORDER BY post_time DESC LIMIT :plimit,10";
$view_posts = $conn->prepare($vpssql);
$view_posts->bindValue(':row_id', $row_id, PDO::PARAM_INT);
$view_posts->bindValue(':pr_onlyme', $pr_onlyme, PDO::PARAM_INT);
...
$vpssql = "SELECT * FROM Post WHERE author_id=:row_id ORDER BY post_time DESC LIMIT :plimit,10";
$view_posts = $conn->prepare($vpssql);
$view_posts->bindValue(':row_id', $row_id, PDO::PARAM_INT);
$view_posts->bindValue(':plimit', (int)trim($plimit), PDO::PARAM_INT);
$view_posts->execute();
