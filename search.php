...
$search_sql = "SELECT * FROM Users WHERE (Fullname LIKE ? OR country LIKE ?) OR (CONCAT(work,?,work0) LIKE ?) ORDER BY country LIMIT 8";
$params = array("%$q%", "%$q%", " ", "%$q%");
...
$csql = "SELECT id FROM Following_Friends WHERE uf_one=:s_id AND uf_two=:id";
$c = $conn->prepare($csql);
...
$all_users_sql = "SELECT * FROM Users ORDER BY id DESC";
...
$csql = "SELECT id FROM Following_Friends WHERE uf_one=:s_id AND uf_two=:id_4User";
$c = $conn->prepare($csql);
$c->bindParam(':s_id',$s_id,PDO::PARAM_INT);
